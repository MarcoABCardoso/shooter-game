...
func _run() -> void:
	...
	var latest := {"movement": Vector2.ZERO}
	controls.input_changed.connect(func(movement: Vector2) -> void:
	...
	_touch(controls, 1, Vector2(1168, 355), true)
	...
	assert(latest["movement"] == Vector2.ZERO, "Hiding controls should release active touch input")
	print("MOBILE_CONTROLS_OK movement stick, ability, and input reset validated")
	quit(0)
