...
func _initialize() -> void:
	...
	assert(not profile.skill_available("arc_conduction"), "Arc Conduction should require native Arc mastery as well as its parent")
	profile._apply_mastery_rewards({"arc": 140.0})
	assert(profile.skill_available("arc_conduction"), "Using Arc should reveal its behavior-changing chain branch")
