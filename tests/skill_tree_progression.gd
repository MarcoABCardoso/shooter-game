...
func _initialize() -> void:
	...
	assert(not profile.skill_available("impact_vector"), "Impact Vector should remain mastery- and stage-gated")
	profile._apply_mastery_rewards({"pulse": 560.0})
	assert(not profile.skill_available("impact_vector"), "Mastery should not bypass the Stage 1 gate")
	profile.clear_stage_one()
	assert(profile.skill_available("impact_vector"), "Stage clear and native Pulse mastery should satisfy the gate")
	assert(profile.buy_skill("impact_vector"), "A revealed second-tier node should be purchasable")
	assert(not profile.skill_available("arc_overload"), "The next branch tier should remain Stage 2 gated")
	...
	print("SKILL_TREE_OK expanded ranks, prerequisites, stage gates, effects, and free respec validated")
	quit(0)
