...
signal destroyed(enemy: NeonEnemy, kind: String, flux: int, xp: int, world_position: Vector2)
...
func take_damage(amount: float, source_weapon: String) -> float:
	...
	if health <= 0.0:
		active = false
		collision_layer = 0
		destroyed.emit(self, kind, flux_reward, xp_reward, global_position)
		queue_free()
	else:
		queue_redraw()
	return dealt
